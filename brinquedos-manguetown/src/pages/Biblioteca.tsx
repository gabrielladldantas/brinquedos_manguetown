function Card({ img, titulo }: { img: string; titulo: string }) {
    return (
        <div className="flex flex-col items-center p-4 border-2 border-[#b8860b] rounded-xl bg-white shadow-md hover:scale-105 transition-transform h-full">
            <img src={img} alt={titulo} className="w-full h-48 object-cover rounded-md mb-4" />
            <h3 className="text-sm font-semibold text-center mb-4 text-gray-800 flex-grow">{titulo}</h3>
...
export default function Biblioteca() {
    return (
        <main className="max-w-7xl mx-auto px-6 py-12 space-y-16">
            <h1 className="text-4xl font-bold text-[#004d40] text-center mb-12">BIBLIOTECA</h1>

            <section>
                <h2 className="text-2xl font-bold mb-8 text-gray-700 border-b-2 border-gray-200 pb-2">Negócios</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
                    {/* Removida a barra inicial para testar acessibilidade direta da pasta public */}
                    <Card img="imagens/1economizando.jpg" titulo="Economizando Dinheiro" />
                    <Card img="imagens/2mulheres.jpg" titulo="Mulheres & Negócios" />
                    <Card img="imagens/3economia.jpg" titulo="Economia Criativa" />
                    <Card img="imagens/4planejamento.jpg" titulo="Planejamento Financeiro" />
                </div>
            </section>

            <section>
                <h2 className="text-2xl font-bold mb-8 text-gray-700 border-b-2 border-gray-200 pb-2">Mulheres</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
